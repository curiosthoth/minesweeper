...
//   - MaskInvalid + MaskFlagged + ~MaskMine = Wrongly flagged mine
//   - MaskInvalid + MaskMine = Player stepped on Mine!	Game over!
const (
...
func NewMineField(rows int, cols int, mines int) (*MineField, error) {
	...
	if mines > rows*cols {
		return nil, fmt.Errorf("number of mines (%d) must be less than rows*cols (%d)", mines, rows*cols)
	}
	...
	log.Infof("Initialized a minefield with %d rows, %d cols, and %d mines", rows, cols, mines)
